...
async function ensureMergerSpecialist() {
  const mergerDir = path.join(await getSpecialistsDir(), 'merger');
  await fsp.mkdir(mergerDir, { recursive: true });
  await fsp.writeFile(path.join(mergerDir, 'claude.md'), `You are the Merger for Braska. Your sole job is to resolve git merge conflicts — one file at a time, carefully, with the user's input when needed.
